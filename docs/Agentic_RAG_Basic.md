...
>
> ※ **Graph RAG**는 문서 간 관계를 그래프 구조로 연결하여 훨씬 복잡한 다단계 추론을 가능하게 하는 고급 기법입니다. 최근 주목받고 있지만, **이 강의의 범위 밖**입니다. Agentic RAG를 충분히 이해한 뒤 다음 단계로 탐색하길 권장합니다.
...
RAG는 문서를 그대로 "읽히게" 하는 게 아니라, "필요한 부분만 찾아 쓸 수 있게" 바꾸는 전처리부터 시작합니다. 그 핵심이 **청킹(Chunking)**입니다.

<a id="sec-1-4-1"></a>

### 1.4.1 청킹 한 줄 정의

- 긴 문서를 LLM이 다루기 좋은 크기의 텍스트 조각(Chunk)으로 나누고, 각 조각에 **출처/위치/제목 같은 메타데이터**를 붙여서 나중에 **검색 → 인용 → 근거 제시**가 가능하게 만드는 전처리 과정입니다.
...
- **방식**: 고정된 크기로 텍스트를 자름
- **장점**: 구현이 간단하고 예측 가능
- **단점**: 문장이나 문단의 의미 경계를 무시하여 중간에 잘릴 위험

#### 2. Recursive Character Text Splitting
- **방식**: 재귀적으로 문단 → 문장 순으로 분할
- **장점**: 의미 경계를 최대한 존중하며 분할
- **단점**: 복잡도가 다소 증가

#### 3. Semantic Chunking
- **방식**: 임베딩 모델을 사용하여 의미적으로 유사한 문장들을 하나의 청크로 묶음
- **장점**: 가장 정교한 의미 단위 분할
- **단점**: 처리 비용과 복잡도가 높음
...
#### 1단계: 데이터 특성 파악
- 샘플 문서를 분석하여 구조적 특성 파악 (헤딩, 표, 반복 문구, 전문 용어 등)

#### 2단계: 전략 실험
- 여러 청킹 전략으로 A/B 테스트
- 예: Fixed vs Recursive, Overlap 유무 비교

#### 3단계: 복합 평가
- 검색 품질 측정 (정확도, 재현율)
- 운영 비용 측정 (지연시간, 저장공간, 인덱싱 시간)
...
비즈니스마다 문서 구조, 전문 용어, 표현 방식이 다르고 (일종의 **조직 특유의 언어/암묵지**), "한 주제"의 경계도 조직마다 다르게 정의됩니다.
따라서 Fixed/Recursive/Semantic 중 하나를 선택하는 단순한 문제가 아니라, **우리 조직의 문서와 질문 패턴에 맞춰** 크기·오버랩·분할 기준을 **복합적이고 전략적으로 설계/튜닝**하는 엔지니어링 문제입니다.
...
ReAct 사이클은 목표를 달성할 때까지 반복되지만, **무한 루프**에 빠지거나 불필요한 비용이 발생하는 것을 막기 위해 반드시 **중단 조건과 안전장치**를 설계해야 합니다.

**중단이 필요한 상황**

| 상황 | 예시 |
|------|------|
| 목표 달성 | LLM이 "Final Answer"를 반환 |
| 도구 실패 반복 | 같은 도구가 3회 연속 에러 |
| 최대 반복 초과 | 10번 돌았는데 답이 안 나옴 |
| 컨텍스트 한도 도달 | 누적 토큰이 한도의 80% 초과 |

**실무에서 사용하는 안전장치**

```python
MAX_ITERATIONS = 10          # 최대 루프 횟수
MAX_TOOL_RETRY = 3           # 도구 실패 시 재시도 한도
CONTEXT_TOKEN_LIMIT = 80000  # 컨텍스트 한도 (모델 최대치의 ~80%)
...
for iteration in range(MAX_ITERATIONS):
    ...
    try:
        ...
        if retry_count >= MAX_TOOL_RETRY:
            observation = "도구 실행에 반복 실패했습니다. 다른 방법으로 답변합니다."
            break  # ⚠️ 도구 실패 한도 → 강제 종료 후 폴백

    prompt = update_prompt(prompt, observation)

else:
    # for-else: 루프가 break 없이 끝나면 실행됨 (최대 반복 초과)
    observation = "최대 반복 횟수에 도달했습니다."
```

**핵심 원칙**

- **루프는 반드시 유한해야 합니다.** 최대 반복 횟수를 설정하지 않으면 LLM이 같은 도구를 계속 호출하다가 비용이 폭증할 수 있습니다.
- **실패 시 폴백(fallback)을 준비합니다.** 도구가 실패했을 때 "모르겠습니다"보다는 "지금 당장 정확한 답은 드리기 어렵지만..."처럼 부분 답변을 제공하는 편이 사용자 경험에 좋습니다.
- **이 강의의 실습 코드**(`chatbot-service/services/tool_executor.py`)에서는 `replan_fn`이 이 역할을 담당합니다 — 도구 실패 시 TaskPlannerAgent에게 새 계획을 요청하고, 재계획도 실패하면 루프를 종료합니다.
...
이 강의의 구현은 에이전트 루프를 **직접 코드로 제어**하는 방식(순수 Python + HTTP)이지만, 실무에서는 **LangGraph** 같은 프레임워크로 동일한 흐름을 구성하는 선택지도 있습니다.

LangGraph는 에이전트의 상태 전이를 **그래프(노드와 엣지)** 로 선언적으로 정의합니다. IntentClassifier → TaskPlanner → ToolExecutor 같은 흐름이 각각 "노드"가 되고, 조건부 분기("단순 질문이면 바로 답변, 복잡하면 재계획")는 "엣지"가 됩니다. 덕분에 흐름이 시각화되고, 중단점(checkpoint) 저장이나 Human-in-the-loop(사람이 중간에 개입) 같은 기능을 비교적 쉽게 붙일 수 있습니다.

반면 이 강의처럼 순수 구현으로 먼저 만들어보면 **"에이전트 루프가 내부적으로 어떻게 돌아가는지"** 를 훨씬 명확하게 이해할 수 있습니다. LangGraph를 나중에 배울 때도 내부 원리를 이미 알고 있기 때문에 훨씬 빠르게 흡수됩니다.

> **정리:** 원리 이해 → 이 강의(순수 구현) / 생산성·확장성 → LangGraph
...
`3.1.2 Re-ranking`에서 설명한 2단계 검색 구조의 핵심인 Re-ranker는, 특히 랭킹의 마지막 10~20% 정확도를 끌어올리는 가장 강력한 레버입니다. re-ranker는 보통 **Cross-Encoder(쿼리-문서 쌍을 같이 읽는 모델)** 계열이 강합니다.
