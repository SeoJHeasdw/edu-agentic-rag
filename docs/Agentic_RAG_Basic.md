...
3.  **임베딩 (Embedding):** 각 텍스트 청크를 `[0.7, -0.2, 0.5, ...]` 와 같은 숫자 벡터(Vector)로 변환합니다. 이 벡터는 '의미의 지도'에서 해당 단락의 위치 좌표와 같습니다. "강아지"와 "멍멍이"는 지도에서 매우 가까운 곳에 위치하게 됩니다.
    ...
    **[심화] 희소 벡터 vs 밀집 벡터: 텍스트를 숫자로 표현하는 두 가지 방식**

    텍스트를 벡터(숫자 배열)로 변환하는 방법은 크게 **희소 벡터(Sparse Vector)**와 **밀집 벡터(Dense Vector)** 두 가지로 나뉩니다.

    **1) 희소 벡터 (Sparse Vector) - 키워드 중심의 통계적 접근**

    희소 벡터는 단어의 등장 **빈도**와 **중요도**를 기반으로 벡터를 만듭니다. 대부분의 값이 0이고, 실제로 등장한 단어 위치만 0이 아닌 값을 가지는 것이 특징입니다.

    *   **동작 방식:** 전체 단어 사전을 기준으로, 문서에 등장한 단어에만 가중치(빈도, TF-IDF 점수 등)를 부여합니다.

    *   **구체적 예시:**
        ```
        전체 단어 사전: ["강아지", "고양이", "귀엽다", "짖다", "야옹"]  (5개 단어)

        문서 A: "강아지는 귀엽다"
        → 희소 벡터: [1, 0, 1, 0, 0]  (강아지=1, 귀엽다=1, 나머지=0)

        문서 B: "고양이는 야옹하고 귀엽다"
        → 희소 벡터: [0, 1, 1, 0, 1]  (고양이=1, 귀엽다=1, 야옹=1, 나머지=0)

        → 벡터의 차원: 단어 사전 크기 (예: 50,000개 단어 = 50,000차원)
        → 대부분의 값이 0 (희소, Sparse)
        ```

    *   **대표 기법:** **TF-IDF**, **BM25** (키워드 기반 검색에서 널리 사용)

    *   **장점:**
        *   정확한 키워드 매칭에 강함 (예: "Python 3.11" 같은 정확한 버전명 검색)
        *   구현이 단순하고, 결과 해석이 직관적
        *   계산 비용이 낮음 (0이 많아 효율적)

    *   **단점:**
        *   의미적 유사성을 이해하지 못함
            *   "강아지"와 "멍멍이"는 의미상 유사하지만, 희소 벡터에서는 완전히 다른 단어 (벡터 간 유사도 0)
        *   동의어, 유의어, 문맥을 고려하지 못함
        *   단어 사전에 없는 신조어나 오타에 취약

    **2) 밀집 벡터 (Dense Vector) - 의미 중심의 학습 기반 접근**

    밀집 벡터는 딥러닝 모델이 대량의 텍스트를 학습하여, 단어나 문장의 **의미**를 숫자로 압축한 표현입니다. 모든 값이 실수(소수점 포함)로 채워져 있으며, 의미적으로 유사한 텍스트는 벡터 공간에서 가까운 위치에 배치됩니다.

    *   **동작 방식:** 사전 학습된 신경망(BERT, GPT, Sentence-BERT 등)을 통해 텍스트를 저차원(보통 384~1536차원)의 의미 벡터로 변환합니다.
    ...
    *   **구체적 예시:**
        ...
        문서 C: "고양이는 우아하다"
        → 밀집 벡터 (384차원): [0.31, 0.62, -0.55, ..., 0.12, 0.74]

        → 벡터의 차원: 모델이 정한 고정 차원 (예: OpenAI ada-002 = 1536차원)
        → 모든 값이 실수로 채워짐 (밀집, Dense)
        → "강아지"와 "멍멍이" 벡터는 코사인 유사도가 높음 (의미적 유사성 반영)
        ```

    *   **대표 모델:** **OpenAI text-embedding-3-small**, **BGE-M3**, **Ko-SimCSE-roberta**, **Sentence-BERT**

    *   **장점:**
        *   의미적 유사성을 정확히 포착
            *   "강아지"와 "멍멍이", "개" 모두 벡터 공간에서 가까운 위치 → 검색 시 동의어도 자동으로 찾아냄
        *   문맥 이해 가능 ("사과(과일)" vs "사과(apology)"를 문맥에 따라 다르게 임베딩)
        *   다국어, 신조어에 강함 (학습 데이터에 기반)

    *   **단점:**
        *   정확한 키워드 매칭에는 약함 (예: "Python 3.11"과 "Python 3.10"을 너무 유사하게 취급할 수 있음)
        *   계산 비용이 높음 (신경망 추론 필요)
        *   결과 해석이 어려움 (벡터 값 자체는 사람이 이해하기 어려움)

    **3) 비교표 요약**

    | 구분 | 희소 벡터 (Sparse Vector) | 밀집 벡터 (Dense Vector) |
    | :--- | :--- | :--- |
    | **표현 방식** | 단어 빈도/통계 기반 | 딥러닝 학습 기반 |
    | **벡터 차원** | 매우 높음 (수만~수십만) | 낮음 (수백~수천) |
    | **0의 비율** | 대부분 0 (Sparse) | 모든 값이 실수 (Dense) |
    | **의미 이해** | 불가능 ("강아지" ≠ "멍멍이") | 가능 ("강아지" ≈ "멍멍이") |
    | **키워드 매칭** | 강함 (정확한 단어 일치) | 약함 (유사 단어도 매칭) |
    | **대표 기법** | TF-IDF, BM25 | BERT, OpenAI Embeddings |
    | **계산 비용** | 낮음 | 높음 (GPU 추천) |
    | **RAG 활용** | 키워드 정확성 중요 시 | 의미 검색이 중요 시 (주류) |

    **4) RAG에서의 실전 활용**

    현대 RAG 시스템에서는 **밀집 벡터(Dense Vector)**가 주로 사용됩니다. 그 이유는 사용자의 질문이 항상 문서와 똑같은 단어로 표현되지 않기 때문입니다. 예를 들어, 사용자가 "반려견 사료 추천"이라고 물어봤을 때, 문서에는 "강아지 먹이 가이드"로 작성되어 있을 수 있습니다. 희소 벡터는 이 둘을 연결하지 못하지만, 밀집 벡터는 의미적 유사성을 통해 연결할 수 있습니다.

    다만, 최신 고급 RAG 시스템에서는 **하이브리드 검색(Hybrid Search)**을 사용하여 두 방식의 장점을 결합하기도 합니다. (자세한 내용은 Chapter 3에서 다룹니다.)
