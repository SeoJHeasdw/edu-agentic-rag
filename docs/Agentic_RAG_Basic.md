...
특히 RAG를 이해하려면 “LLM이 왜 단독으로는 위험해질 수 있는지”를 먼저 감 잡아야 합니다. 아래 4가지는 실무에서 자주 문제로 드러나는 대표 사례입니다.

**먼저 정의부터: RAG가 뭔가요?**

- **RAG(Retrieval-Augmented Generation)**: LLM이 답변을 만들기 전에, **외부 지식(문서/DB/위키/사내 매뉴얼 등)에서 관련 정보를 검색(Retrieval)**하고, 그 내용을 **근거로 답변을 생성(Generation)**하는 방식
...
**마지막으로, Agentic RAG로 왜 넘어가나요? (맛보기)**
