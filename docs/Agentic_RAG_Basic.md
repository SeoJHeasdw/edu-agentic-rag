...
### LLM의 한계: 오픈북 시험이 필요한 이유

대규모 언어 모델(LLM)은 방대한 지식을 학습한 천재이지만, 두 가지 치명적인 약점이 존재합니다.

**LLM의 2가지 한계**

- **오래된 기억 (Knowledge Cut-off)**
  - 2023년에 학습이 끝난 모델은 "2024년 최신 AI 트렌드"에 대해 답변할 수 없음
  - 학습 시점 이후의 정보는 알 수 없음

- **그럴듯한 거짓말 (환각, Hallucination)**
  - LLM은 모르는 내용도 그럴듯하게 지어냄
  - 예: "세종대왕이 맥북을 던진 사건"과 같은 허구도 사실처럼 생성 가능

**RAG의 해결책**

RAG(검색 증강 생성)는 LLM을 **'오픈북 시험'**을 치르는 학생으로 만듭니다.

- LLM이 자신의 기억력에만 의존하지 않음
- 신뢰할 수 있는 최신 자료를 직접 검색
- 검색한 내용을 근거로 정확한 답변 생성

> **핵심 비유**
> - **LLM 단독:** 암기한 내용으로만 시험 보는 '클로즈북 시험'
> - **RAG 활용:** 교과서를 찾아보며 시험 보는 '오픈북 시험'
...
- 각 텍스트 청크를 숫자 벡터로 변환
  - 예: `[0.7, -0.2, 0.5, ...]`
- 이 벡터는 '의미의 지도'에서 해당 텍스트의 위치 좌표
- 의미적으로 유사한 단어들은 벡터 공간에서 가까이 위치
  - 예: "강아지"와 "멍멍이"는 지도에서 매우 가까운 곳에 위치

---

#### 희소 벡터 vs 밀집 벡터: 텍스트를 숫자로 표현하는 두 가지 방식
