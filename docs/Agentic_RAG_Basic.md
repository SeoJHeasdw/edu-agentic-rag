...
- 하지만 실제 문제는 종종 다음이 필요합니다:
  - 질문이 애매하면 **되물어서 요구사항을 명확히 하기**
...
이 파이프라인에서 특히 **임베딩/유사도 검색**을 이해하려면, 텍스트를 숫자로 바꾸는 “벡터 표현”을 먼저 알아야 합니다.
...
지금까지는 “텍스트를 벡터로 바꾸면 유사도를 계산할 수 있다”는 **검색의 재료(표현 방식)**를 봤습니다.  
이제부터는 이 재료를 실제 제품(서비스) 안에서 어떻게 조립해서 **RAG 시스템이 동작**하는지를 보겠습니다.

> 참고(예고): 희소/밀집을 섞어서 쓰는 **하이브리드 검색** 같은 고급 기법은, RAG 파이프라인을 한 번 이해한 뒤에(Chapter 3) 다루는 편이 훨씬 직관적입니다.
...
에이전트/Agentic RAG는 기본 RAG가 어느 정도 손에 잡힌 뒤에(Chapter 3) 소개하는 편이 훨씬 이해가 빠릅니다.
...
```python
"""
이 레포의 실습 구현은 LangChain 벡터스토어를 직접 쓰지 않고,
